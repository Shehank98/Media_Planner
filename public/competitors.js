...
function renderAnalysis(r) {
  ...
  // --- the written analysis, before the charts --------------------------
...
function stackedChart(rows, fmtVal = money) {
      ...
      return el('div', { class: 'ca-barrow' },
        ...
        el('div', { class: 'ca-track ca-stack', style: `width:${Math.max(2, (total / max) * 100)}%` },
          ...r.segments.filter((s) => s.value > 0).map((s) => {
            ...
            // Label the segment inline once it is wide enough to hold the text.
            return el('div', {
              class: 'ca-seg', title: `${r.label}: ${s.hover} (${share}%)`,
              style: `flex:${s.value};background:${s.color}`,
            }, share >= 12 ? el('span', { class: 'ca-seglabel' }, `${share}%`) : null);
          })),
